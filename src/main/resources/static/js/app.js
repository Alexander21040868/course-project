document.addEventListener('DOMContentLoaded', () => {
    ...
    function navigateTo(view) {
        ...
        const titles = { quests:'Карта подземелий', lesson:'Подземелье', task:'Битва', challenges:'Арена', achievements:'Зал славы', profile:'Личный кабинет', admin:'Штаб мастера' };
    ...
    async function loadFullProfile() {
        ...
        try {
            const p = await API.get('/profile');
            state.profile = p;
            const pctTotal = p.totalTasks > 0 ? Math.round(p.solvedCount * 100 / p.totalTasks) : 0;
            el.innerHTML = `
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card"><div class="stat-val">${p.solvedCount}/${p.totalTasks}</div><div class="stat-label">Решено задач</div></div>
                    <div class="stat-card"><div class="stat-val">${p.achievements.length}</div><div class="stat-label">Достижений</div></div>
                </div>
                ...
                <div class="history-list">${p.recentSubmissions.length ? p.recentSubmissions.map(s => `
                    <div class="history-item">
                        <span class="history-status ${s.status === 'CORRECT' ? 'ok' : 'fail'}">${s.status === 'CORRECT' ? '✅' : '❌'}</span>
                ...
                <div class="achievements-grid">${p.achievements.length ? p.achievements.map(a => `
                    <div class="ach-card"><div class="ach-icon">${a.icon}</div><div class="ach-info"><h4>${esc(a.name)}</h4><p>${esc(a.description)}</p><div class="ach-xp">+${a.xpReward} XP</div></div></div>`).join('') : '<p style="color:var(--text-dim);font-size:12px">Решайте задачи, чтобы получить награды!</p>'}</div>`;
    ...
    async function loadLessons() {
        ...
        try {
            ...
            if (!lessons.length) { el.innerHTML = '<div class="empty-state"><div class="empty-icon">🗺</div><p>Подземелья ещё не созданы.</p></div>'; return; }
            el.innerHTML = lessons.map(l => `
                <div class="quest-card" data-id="${l.id}">
                    <div class="quest-order">ПОДЗЕМЕЛЬЕ #${l.orderIndex + 1}</div>
                    <h3>${esc(l.title)}</h3>
                    <p>${esc(l.description || '')}</p>
                    <div class="quest-meta"><span>⚔ ${l.taskCount} задач</span><span>👤 ${esc(l.authorName)}</span></div>
                </div>`).join('');
    ...
    async function openLesson(id) {
        ...
        navigateToView('lesson'); $('viewTitle').textContent = 'Подземелье';
        try {
    ...
    async function openTask(id) {
        ...
        navigateToView('task'); $('viewTitle').textContent = 'Битва';
        ...
        try {
            ...
            if (t.hints) { $('hintBox').style.display = ''; $('hintText').textContent = t.hints; } else { $('hintBox').style.display = 'none'; }
            if (t.sampleTests && t.sampleTests.length) {
                $('sampleTestsBlock').style.display = '';
                $('sampleTestsList').innerHTML = t.sampleTests.map((tc, i) => `
    ...
    $('submitBtn').addEventListener('click', async () => {
        ...
        try {
            ...
            if (res.testResults && res.testResults.length > 0) {
                $('resultTests').innerHTML = '<div class="test-results">' + res.testResults.map(tr =>
                    `<span class="test-badge ${tr.passed?'pass':'fail'}" title="${tr.sample && !tr.passed ? 'Ожидалось: '+esc(tr.expected||'') : ''}">#${tr.testNumber} ${tr.passed?'✓':'✗'}</span>`
                ).join('') + '</div>';
            } else { $('resultTests').innerHTML = ''; }
    ...
    async function loadChallenges() {
        ...
        try {
            ...
            el.innerHTML = chs.map(c => {
                ...
                return `<div class="challenge-card ${c.active?'':'ended'}">
                    ...
                    <div class="ch-meta">
                        ...
                        <span>⏱ ${c.active ? remaining + 'ч осталось' : 'Завершён'}</span>
                        <span>👤 ${esc(c.createdByName)}</span>
                    </div>
                    <div class="ch-actions">
                        ${c.active && !c.joined ? `<button class="btn-join" data-id="${c.id}">Вступить в бой</button>` : ''}
            ...
            el.querySelectorAll('.btn-results').forEach(b => b.addEventListener('click', async () => {
                const resEl = $('ch-results-' + b.dataset.id);
                if (resEl.style.display !== 'none') { resEl.style.display = 'none'; return; }
                try {
                    const results = await API.get('/challenges/' + b.dataset.id + '/results');
                    resEl.innerHTML = results.length ? '<table class="results-table"><tr><th>#</th><th>Герой</th><th>Решено</th><th>LVL</th></tr>' +
                        results.map(r => `<tr><td>${r.rank}</td><td>${esc(r.username)}</td><td>${r.tasksSolved}</td><td>${r.level}</td></tr>`).join('') + '</table>'
                        : '<p style="font-size:12px;color:var(--text-dim)">Пока нет участников</p>';
                    resEl.style.display = '';
                } catch(e) { resEl.innerHTML = '<p>Ошибка</p>'; resEl.style.display = ''; }
            }));
        } catch (e) { el.innerHTML = `<div class="empty-state"><p>Ошибка: ${esc(e.message)}</p></div>`; }
    }
    ...
    async function loadAchievements() {
        ...
        try {
            ...
            if (!achs.length) { el.innerHTML = '<div class="empty-state"><div class="empty-icon">🏆</div><p>Решайте задачи, чтобы получить награды!</p></div>'; return; }
    ...
    async function loadStudents() {
        ...
        try {
            ...
            el.innerHTML = '<table class="results-table"><tr><th>Герой</th><th>XP</th><th>LVL</th><th>Решено</th><th>Прогресс</th></tr>' +
                students.map(s => `<tr>
                    <td>${esc(s.username)}</td><td>${s.xp}</td><td>${s.level}</td>
    ...
    $('createLessonForm').addEventListener('submit', async e => {
        ...
        try { await API.post('/admin/lessons', { title: fd.get('title'), description: fd.get('description'), content: fd.get('content'), orderIndex: +fd.get('orderIndex') }); e.target.reset(); showToast('📖','Урок создан','Новое подземелье на карте'); } catch(err) { alert(err.message); }
    });
    ...
    $('createChallengeForm').addEventListener('submit', async e => {
        ...
        try {
            ...
            e.target.reset(); showToast('🏟','Челлендж создан','Герои могут вступить в бой');
        } catch(err) { alert(err.message); }
    });
    ...
    $('codeEditor').addEventListener('keydown', e => {
        if (e.key === 'Tab') { e.preventDefault(); const s = $('codeEditor').selectionStart; const end = $('codeEditor').selectionEnd; $('codeEditor').value = $('codeEditor').value.substring(0,s)+'    '+$('codeEditor').value.substring(end); $('codeEditor').selectionStart = $('codeEditor').selectionEnd = s+4; }
    });
